...
- **トランザクション境界**:
    ...
    - フェーズ2: ドキュメント保存+インデックス再構築（単一トランザクション）
      - 以下の順序で実行する:
        ...
        3. VectorStore.clear() - 既存のベクトルエントリを全削除
        4. VectorStore.add() - 新規のベクトルエントリを全挿入
      - エラー発生時はロールバックされ、既存のインデックスが維持される
      - 外部キー制約（ON DELETE CASCADE）により、ドキュメント削除時に関連するベクトルエントリも削除されるが、明示的にVectorStore.clear()も実行することで一貫性を保つ
