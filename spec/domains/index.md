...
- **主要な関心事**: API連携、形式変換（JSON/HTML/テキスト）

### 3. VectorIndex

ベクトルインデックスの管理と質問応答に関するドメイン。RAGフレームワークを抽象化し、インデックスの構築・検索・回答生成を担当する。

- **責務**: インデックスの構築・管理・検索、LLMによる回答生成、回答エンティティの管理
- **主要な関心事**: チャンク分割、埋め込み生成、ベクトルストア操作、LLM連携

## ドメイン間の依存関係

```
┌─────────┐     ┌─────────────┐     ┌──────────┐
│ Message │     │ VectorIndex │     │ Document │
└─────────┘     └─────────────┘     └──────────┘
      │                │                   │
      │                │                   │
      └────────────────┼───────────────────┘
...
- 3つのドメインは互いに独立しており、直接の依存関係はない
- 各ドメインはsharedの共通型（DocumentId, SimilarityScore）を参照する
- 各ドメインはアプリケーション層のユースケースによってオーケストレーションされる
...
1. **テナントIDの追加**:
   ...
   - `documents`, `vector_index_entries`
