...
本システムは以下の5つのドメインで構成される。

### 1. Config

アプリケーション設定の管理に関するドメイン。環境変数から設定を読み込み、型安全な設定オブジェクトとして提供する。

- **責務**: 設定の読み込み・バリデーション・提供
- **主要な関心事**: 環境変数管理、型安全な設定オブジェクト、リトライポリシー
...
アプリケーション設定はドメイン間の依存関係には含まれない。設定値はTypeScriptの型付き定数として定義され、DIコンテナ生成時に各アダプター実装に注入される。ドメイン層は設定への直接的な依存を持たない。
